...
const registry = new EntitySchemaRegistry()
  .register({
    ...
    table: "user",
    collections: {
      posts: { type: "owned" },           // 1:N - User owns Posts
      followers: { 
        type: "reference",                 // N:N - User references other Users
        entity: "User" 
      },
    },
  })
  .register({
    ...
    collections: {
      comments: { type: "owned" },         // 1:N - Post owns Comments
      tags: { 
        type: "reference",                 // N:N - Post references Tags
        entity: "Tag",
        // Optional: junction table config (Prisma handles this automatically)
        junction: {
          table: "post_tags",
          sourceKey: "post_id",
          targetKey: "tag_id",
