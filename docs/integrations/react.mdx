...
import { FilterDemo } from "/snippets/filter-demo.mdx"
...
function UserList() {
  ...
  const fetchUsers = async () => {
    const response = await fetch(`/api/users?${new URLSearchParams(criteria.toJSON())}`);
...
interface UseCriteriaReturn<T> {
  ...
  // Utility methods
  reset: () => void;
  getFilterByField: (field: FieldPath<T>) => Filter<string, unknown> | undefined;
...
function UserList() {
  const { criteria, filters, setPage, addFilter, clearFilters } = useCriteria<User>({
    pageSize: 10,
  ...
  return (
    <div>
      ...
      {/* Pagination */}
      <button onClick={() => setPage(data.meta.page - 1)} disabled={!data.meta.hasPrevious}>
        Previous
      </button>
      <span>Page {data.meta.page} of {data.meta.totalPages}</span>
      <button onClick={() => setPage(data.meta.page + 1)} disabled={!data.meta.hasNext}>
        Next
      </button>
    </div>
  );
...
function UserFilters() {
  const { filters, addOrReplaceByIndex, removeFilter, clearFilters } = useCriteria<User>();
...
async function fetchProducts(criteria: Criteria<Product>) {
  ...
  if (orders.length > 0) {
    params.set("orderBy", orders.map((o) => `${o.field}:${o.direction}`).join(","));
  }
...
<Info>
  The Filter component is built with shadcn/ui and fully customizable. Try the interactive demo below:
</Info>

<FilterDemo />

The Filter component provides:

- **Field Selection**: Dropdown to choose which field to filter
- **Operator Selection**: Context-aware operators based on field type
- **Value Input**: Appropriate input based on operator (text, select, date picker, etc.)
- **Multi-value Support**: For `in` and `notIn` operators
- **Date Range**: For `between` operator on date fields
- **Active Filter Pills**: Visual display of active filters with remove buttons
- **Generated Criteria Preview**: See the code that would be generated (shown in the demo above)

<Tip>
  You can customize the Filter component's appearance by modifying the installed source files. Since you own the code, you have full control over styling and behavior.
</Tip>
