...
An entity should represent a single concept. If an entity is growing too large, consider splitting it into multiple entities or extracting Value Objects.
</Accordion>

<Accordion title="Encapsulate state changes">
Don't expose setters directly. Use methods that express domain operations:

```typescript
// ❌ Avoid
item.props.quantity = 5;
...
<Accordion title="Validate invariants">
Use validation schemas and hooks to ensure entities are always in a valid state.
</Accordion>

<Accordion title="Access through Aggregate">
Don't load entities directly from repositories. Access them through their parent Aggregate.
</Accordion>
