...
keywords: ["value-object","immutable","primitive","validation","ddd","domain-driven-design","typescript"]
---
...
class Status extends ValueObject<string> {
  protected static hooks: VOHooks<string, Status> = {
    onCreate: (status) => {
      if (status.value === "completed") {
        status.addDomainEvent({
...
const status = new Status("completed");
const events = status.getUncommittedEvents();
// [{ type: "status.completed", payload: { value: "completed" } }]
```
...
<Accordion title="Use ValueObjects for domain concepts">
  Wrap primitive values that have domain meaning (Email, Price, Percentage) rather
  than using plain strings/numbers.
</Accordion>

<Accordion title="Add validation">
  Always validate ValueObjects to ensure they contain valid values.
</Accordion>

<Accordion title="Encapsulate behavior">
  Put domain logic related to the value inside the ValueObject class.
</Accordion>

<Accordion title="Keep them simple">
  ValueObjects should wrap a single primitive value. For complex objects with multiple
  properties, use Entities instead.
</Accordion>

<Accordion title="Use factory methods">
  Provide static factory methods for common creation patterns.
</Accordion>
