...
<Accordion title="Always use markAsClean() after persistence">
After successfully persisting changes, always call `markAsClean()` to reset the tracking state:
...
Otherwise, the same changes will be detected again on the next `getChanges()` call.
</Accordion>

<Accordion title="Use typed changes for safety">
Define an entity map and use `getTypedChanges()` for autocomplete and type inference:

```typescript
class Order extends Aggregate<OrderProps> {
  getTypedChanges() {
    type Entities = { Order: Order; OrderItem: OrderItem; };
    return this.getChanges<Entities>();
  }
}
```
...
<Accordion title="Process batch operations in order">
Always process batch operations in this order:
1. Deletes (automatically sorted leaf → root)
2. Creates (automatically sorted root → leaf)  
3. Updates (any order)

```typescript
const batch = changes.toBatchOperations();

await processDeletes(batch.deletes);  // First
await processCreates(batch.creates);  // Second
await processUpdates(batch.updates);  // Last
```
</Accordion>

<Accordion title="Define identity keys for trackable Value Objects">
If you need to track additions/removals of Value Objects in collections, define an identity key:
...
class Like extends ValueObject<LikeProps> {
  static readonly identityKey = ["postId", "userId"]; // Composite
}
```
</Accordion>

<Accordion title="Use transactions for batch operations">
Wrap batch operations in a database transaction to ensure atomicity:
...
aggregate.markAsClean(); // Only after successful commit
```
</Accordion>
