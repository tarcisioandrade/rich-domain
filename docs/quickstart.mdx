...
<Note>
  rich-domain works with any Standard Schema compatible library: Zod, Valibot, ArkType, and others.
</Note>
...
Use `Id.from()` when loading from database:

```typescript
// From database
const data = await db.products.findById("product-123");

const product = new Product({
  id: Id.from(data.id), // Marks as existing
  name: data.name,
  price: data.price,
  stock: data.stock,
  status: data.status,
