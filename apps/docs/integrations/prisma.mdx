...
<Note>
  For a complete working example, see the [fastify-with-prisma
  example](https://github.com/tarcisioandrade/rich-domain/tree/main/examples/backend/fastify-with-prisma)
  in the repository.
</Note>
...
<Info>
  See the complete [Schema Registry documentation](/repository/schema-registry)
  for all features.
</Info>
...
| Aspect          | Registry `fields`                          | BatchExecutor `dataMappers`   |
| --------------- | ------------------------------------------ | ----------------------------- |
| **Type**        | Declarative config                         | Custom functions              |
| **Scope**       | All operations (create, update, delete)    | CREATE operations only        |
| **Usage**       | Automatic via `mapEntity()`, `mapFields()` | Opt-in (optional override)    |
| **Purpose**     | Map field names (domain → database column) | Complex entity transformation |
| **Reusability** | Defined once, used everywhere              | Per-executor instance         |

<Tip>
  **When to use each:**

**Registry `fields`** - Simple field name conversions applied automatically:
...
**`dataMappers`** - Custom transformation logic when default mapping isn't enough:
...
dataMappers: {
  Comment: (item) => ({
    ...
    wordCount: item.data.text.split(" ").length, // Computed field
    status: item.data.approved ? "published" : "draft", // Conditional
  }),
}
```

If no `dataMapper` is defined, the executor uses the registry's `mapEntity()` as fallback.
