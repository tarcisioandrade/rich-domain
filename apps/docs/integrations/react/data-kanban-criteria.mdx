...
<Note>
  The `value` property is used when the column `id` differs from the actual
  value stored in your data. For example, if your column has `id: "in-progress"`
  but your database stores `status: "DOING"`, set `value: "DOING"`. When a card
  is moved to this column, the hook will use `value` for optimistic updates. If
  `value` is omitted, the column `id` is used instead.
</Note>
...
<DataKanbanCriteria
  ...
  renderEmptyState={(column) => (
    <div className="flex flex-col items-center justify-center py-8 text-muted-foreground">
      ...
      <button className="mt-2 text-primary text-sm">+ Add task</button>
    </div>
  )}
/>
```
