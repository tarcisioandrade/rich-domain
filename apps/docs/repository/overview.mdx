...
class UserRepository extends Repository<User> {
  ...
  async countByStatus(status: "active" | "inactive"): Promise<number> {
    return this.count(Criteria.create<User>().whereEquals("status", status));
  }
}
```
...
<Tip>
  The [Prisma Adapter](/integrations/prisma) provides `PrismaBatchExecutor` that
...
describe("UserService", () => {
  ...
  it("should find active users", async () => {
    ...
    await userRepository.save(
      new User({ name: "Inactive", status: "inactive" })
    );
...
<CardGroup cols={2}>
  ...
  <Card
    ...
    icon="sitemap"
    href="/repository/schema-registry"
  >
