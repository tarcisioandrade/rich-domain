...
keywords: ["change-tracking","proxy","audit","history","state-management","ddd","typescript"]
---
...
When persisting an entity for the first time, use `markAsPersisted()` instead of `markAsClean()`. This method also marks the Id as not-new, ensuring subsequent saves trigger UPDATE operations instead of INSERT:

```typescript
// First save (INSERT)
await repository.save(order);

// Mark as persisted - clears changes AND marks Id as not-new
order.markAsPersisted();

console.log(order.isNew());          // false (was true before)
console.log(order.getChanges().isEmpty()); // true

// Now future saves will be UPDATEs
order.updateStatus("confirmed");
await repository.save(order);  // This triggers an UPDATE, not INSERT
```

<Info>
`markAsPersisted()` recursively marks all nested entities as persisted too.
Use `markAsClean()` when you only need to reset change tracking without affecting the Id state (e.g., after UPDATEs).
</Info>
...
For tracking changes in collections of complex objects, use Entities instead of ValueObjects:
...
const changes = order.getTypedChanges();
const items = changes.for("OrderItem"); // Autocomplete works!
```
</Tip>
