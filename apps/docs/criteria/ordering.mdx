...
const criteria = Criteria.create<User>().paginate(3, 10); // Page 3, 10 items per page (offset: 20)
```
...
```typescript
const criteria = Criteria.create<User>().limit(5); // First 5 results
...
const criteria = Criteria.create<User>().paginate(3, 25);
...
```typescript
const criteria = Criteria.create<User>().paginate(1, 20);
...
<Note>
  When search is applied with `PaginatedResult.fromArray()`, it: 1. Searches
  across **all** items (ignoring pagination initially) 2. Returns matching items
  3. Updates the total count based on matches 4. Then applies pagination to the
  filtered results
</Note>

```typescript
// 1000 users in database
const criteria = Criteria.create<User>().search("john").paginate(1, 10);
...
function buildProductQuery(filters: ProductFilters): Criteria<Product> {
  let criteria = Criteria.create<Product>().whereEquals("status", "active");
...
```typescript
// URL: /products?status:equals=active&price:lessThan=100&search=laptop&orderBy=price:asc&page=1&limit=20
