...
class User extends Aggregate<
  UserProps,
  "password" | "passwordResetToken" | "createdAt"
> {
...
<Note>
  Values generated in `onBeforeCreate` are still validated by the schema. If
  generation fails to meet schema requirements, a `ValidationError` will be
  thrown.
</Note>
