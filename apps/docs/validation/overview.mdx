...
keywords: ["validation","schema","standard-schema","zod","valibot","arktype","typescript"]
---
...
interface ValidationConfig {
  ...
  lockMutationsWhenInvalid?: boolean; // Block all mutations while errors exist (default: false)
}
```
...
When `throwOnError` is `false`, mutations are allowed by default so users can fix invalid fields. Set `lockMutationsWhenInvalid: true` to freeze the entity until all validation errors are resolved.
