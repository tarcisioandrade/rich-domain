...
<Tip>
  **Registry Fields vs DataMappers - When to use each?**
...
**Use Registry `fields`** for simple field name conversions:

```typescript
fields: { content: "main_content", createdAt: "created_at" }
```

**Use `dataMappers`** when you need custom transformation logic:
...
If no `dataMapper` is defined for an entity, the executor falls back to the registry's `mapEntity()` method.
...
const post = Post.restore({
  ...
  published: false,
  authorId: user.id.value,
  tags: [new Tag({ id: Id.from("tag-1") }), new Tag({ id: Id.from("tag-2") })],
  createdAt: new Date(),
  updatedAt: new Date(),
});
...
<Tip>
  The CLI analyzes your Prisma schema relationships and generates proper DDD
  aggregates. See the [CLI documentation](/CLI) for more details.
</Tip>
