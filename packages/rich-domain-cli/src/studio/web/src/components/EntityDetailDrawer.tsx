...
export default function EntityDetailDrawer({
  ...
  const Icon = entity.type === 'aggregate' ? Box : entity.type === 'value-object' ? Gem : Layers;
  const iconColor = entity.type === 'aggregate' ? 'text-primary' : entity.type === 'value-object' ? 'text-emerald-400' : 'text-blue-400';
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
            ...
            <div className="bg-muted/30 rounded p-2 space-y-1">
              {entity.properties.map((prop, i) => (
                <div key={i} className="flex items-center gap-2 text-xs font-mono">
                  <span>{prop.name}</span>
                  <span className="text-muted-foreground">{prop.optional ? '?:' : ':'}</span>
                  <span className="text-blue-400">{prop.type}</span>
                </div>
              ))}
            </div>
          ) : (
            <p className="text-xs text-muted-foreground italic">No properties detected</p>
          )}
        </section>
              ...
              {entity.relationships.map((rel, i) => (
                <div key={i} className="bg-muted/30 rounded p-2 text-xs">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="font-mono font-semibold">{rel.propertyName}</span>
                    <span className={cn(
                      'px-1.5 py-0.5 rounded text-[10px]',
                      rel.relationshipType === 'composition' ? 'bg-emerald-500/20 text-emerald-400' :
                      rel.relationshipType === 'reference' ? 'bg-blue-500/20 text-blue-400' :
        ...
        <section>
          ...
          {entity.methods.length > 0 ? (
            ...
            <p className="text-xs text-muted-foreground italic">No methods detected</p>
          )}
        </section>
      </div>
    </div>
